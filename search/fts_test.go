...
func seedVerses(t *testing.T, idx *FTS) {
	t.Helper()
...
// TestKeywordSearchWithoutOpenAI is the guarantee that matters most: with no
...
func TestKeywordSearchWithoutOpenAI(t *testing.T) {
	idx := NewFTS()
	defer idx.Close()
	seedVerses(t, idx)
